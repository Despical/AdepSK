package me.adeptr.adepsk.holo.skript.conditions;
...
@Examples({ "if holo {_variable} is hidden from the player:", "if all players can see holograms {_list::*}:",
		"if hologram {_holo} is not visible for {_player}:", "if the player can see the holo {holograms::1}:" })
@Since("1.0.0")
...
public class CondCanSeeHologram extends Condition {
	...
	public boolean check(Event e) {
		return players.check(e, player -> holograms.check(e, holo -> holo.getVisibilityManager().isVisibleTo(player)),
				isNegated());
	}

	@Override
	public String toString(@Nullable Event e, boolean debug) {
		return players.toString(e, debug) + " can " + (isNegated() ? "not " : "") + "see"
				+ holograms.toString(e, debug);
	}
