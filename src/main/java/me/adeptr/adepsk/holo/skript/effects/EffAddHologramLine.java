...
@Examples({ "append a stone to {_holo}", "prepend \"<light green>Test\" to the created hologram",
...
public class EffAddHologramLine extends Effect {
	...
	public String toString(@Nullable Event e, boolean debug) {
		...
		if (clickable && touchable)
			...
			interact = "touchable ";
		if (mode == Modes.INSERT)
			return "insert " + (interact != null ? interact + " lines" : "") + lines.toString(e, debug) + " into "
					+ holograms.toString(e, debug) + " at line " + line.toString(e, debug);
		else
			return (mode == Modes.PREPEND ? "prepend " : "append ") + (interact != null ? interact + " lines" : "")
					+ lines.toString(e, debug) + " to " + holograms.toString(e, debug) + " at line "
					+ line.toString(e, debug);
	}
