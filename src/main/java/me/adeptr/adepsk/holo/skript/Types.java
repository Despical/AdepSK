package me.adeptr.adepsk.holo.skript;
...
public class Types {
	...
	static {
		// Hologram
		Classes.registerClass(new ClassInfo<>(Hologram.class, "hologram").user("holo(gram)?s?").name("Hologram")
				...
				.since("1.0.0").changer(new Changer<Hologram>() {
					@Override
		...
		Converters.registerConverter(Hologram.class, Location.class,
				(Converter<Hologram, Location>) Hologram::getLocation);
		...
		Classes.registerClass(new ClassInfo<>(HologramLine.class, "hologramline").user("holo(gram)?( |-)?lines?")
				.name("Hologram Line")
				.description(
						"A line of a HolographicDisplays hologram. Can be deleted using the 'delete/clear' changer.",
						"",
				...
				.since("1.0.0").changer(hologramLineChanger).parser(new Parser<HologramLine>() {
					@Override
					public boolean canParse(ParseContext context) {
						return false;
					}
		...
		Converters.registerConverter(ItemLine.class, ItemType.class,
				(Converter<ItemLine, ItemType>) line -> new ItemType(line.getItemStack()));
