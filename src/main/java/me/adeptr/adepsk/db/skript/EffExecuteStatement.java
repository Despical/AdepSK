package me.adeptr.adepsk.db.skript;
...
import me.adeptr.adepsk.db.SkriptUtil;

/**
 * Executes a statement on a database and optionally stores the result in a
 * variable. Expressions embedded in the query will be escaped to avoid SQL
 * injection.
 * <p>
 * If a single variable, such as `{test}`, is passed, the variable will be set
 * to the number of affected rows.
 * <p>
 * If a list variable, such as `{test::*}`, is passed, the query result will be
 * mapped to the list variable in the form `{test::<column name>::<row number>}`
 *
 * Specifying `synchronously` will make skript-db execute the query on the event
 * thread, which is useful for async events. Note that skript-db will ignore
 * this flag if you attempt to run this on the main thread.
 *
 * @name Execute Statement
 * @pattern [synchronously] execute %string% (in|on) %datasource% [and store
 *          [[the] (output|result)[s]] (to|in) [the] [var[iable]] %-objects%]
 * @example execute "select * from table" in {sql} and store the result in
 *          {output::*}
 * @example execute "select * where player=%{player}%" in {sql} and store the
 *          result in {output::*}
 * @since 0.1.0
 */
public class EffExecuteStatement extends Delay {
	static {
		Skript.registerEffect(EffExecuteStatement.class, "[(1¦synchronously)] execute %string% (in|on) %datasource% "
	...
	protected void execute(Event e) {
		...
		if (isSync && !isMainThread) {
			...
			CompletableFuture<String> sql = CompletableFuture.supplyAsync(() -> executeStatement(e), threadPool);
			...
			sql.whenComplete((res, err) -> {
				...
				Bukkit.getScheduler().runTask(Main.getInstance(), () -> continueScriptExecution(e, res));
			});
		}
	}
	...
	private String executeStatement(Event e) {
		...
		try (Connection conn = ds.getConnection(); PreparedStatement stmt = createStatement(e, conn)) {

			boolean hasResultSet = stmt.execute();

			if (var != null) {
				String baseVariable = var.toString(e).toLowerCase(Locale.ENGLISH);
				if (isList) {
					baseVariable = baseVariable.substring(0, baseVariable.length() - 1);
				}

				if (hasResultSet) {
					CachedRowSet crs = Main.getRowSetFactory().createCachedRowSet();
			...
			if (o instanceof String) {
				...
				if (expr instanceof ExprUnsafe) {
					...
					if (standaloneString && expressionValue instanceof String) {
						...
						Skript.warning(String.format(
								"Unsafe may have been used unnecessarily. Try replacing 'unsafe %1$s' with %1$s",
								rawExpression));
					}
				} else {
	...
	private void populateVariable(Event e, CachedRowSet crs, String baseVariable) throws SQLException {
		...
		while (crs.next()) {
			for (int i = 1; i <= columnCount; i++) {
				setVariable(e, baseVariable + meta.getColumnLabel(i).toLowerCase(Locale.ENGLISH) + Variable.SEPARATOR
						+ rowNumber, crs.getObject(i));
			}
			rowNumber++;
		}
	}
