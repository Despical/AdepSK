...
public class CustomExpressionSection extends CustomSyntaxSection<ExpressionSyntaxInfo> {
	...
	static {
		...
		Optional<ExpressionInfo<?, ?>> info = StreamSupport
				.stream(Spliterators.spliteratorUnknownSize(Skript.getExpressions(), Spliterator.ORDERED), false)
				.filter(i -> i.c == CustomExpression.class).findFirst();
	...
	protected boolean init(Literal[] args, int matchedPattern, SkriptParser.ParseResult parseResult, SectionNode node) {
		...
		case 1:
			...
			for (Node subNode : patterns) {
				register(ExpressionSyntaxInfo.create(script, subNode.getKey(), i++, alwaysPlural, false, false));
			}
			break;
		case 2:
			what = parseResult.regexes.get(0).group();
			String fromType = Arrays.stream(((Literal<ClassInfo>) args[0]).getArray()).map(ClassInfo::getCodeName)
					.map(codeName -> {
			...
			register(ExpressionSyntaxInfo.create(script, "[the] " + what + " of %" + fromType + "%", 1, alwaysPlural,
					true, true));
			register(ExpressionSyntaxInfo.create(script, "%" + fromType + "%'[s] " + what, 1, alwaysPlural, false,
					true));
			break;
		}
		...
		return handleEntriesAndSections(node, entryNode -> {
			...
			for (Changer.ChangeMode mode : Changer.ChangeMode.values()) {
				...
				if (key.startsWith(name)) {
					...
					whichInfo.forEach(which -> {
						Map<Changer.ChangeMode, Trigger> changerMap = changerHandlers.computeIfAbsent(which,
								k -> new HashMap<>());

						changerMap.put(mode, new Trigger(SkriptUtil.getCurrentScript(),
								String.format("%s %s", name, which.getPattern()), this, items));
					});

					if (rawTypes.length() > 0) {
						Class[] acceptedClasses = Arrays.stream(rawTypes.split(",")).map(String::trim)
								.map(SkriptUtil::getUserClassInfoAndPlural).map(meta -> {
						...
						whichInfo.forEach(which -> {
							changerTypes.computeIfAbsent(which, k -> new HashMap<>()).put(mode, acceptedClasses);
						});
					}
