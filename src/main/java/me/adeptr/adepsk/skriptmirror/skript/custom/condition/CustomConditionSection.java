package me.adeptr.adepsk.skriptmirror.skript.custom.condition;
...
public class CustomConditionSection extends CustomSyntaxSection<ConditionSyntaxInfo> {
	static {
		...
		CustomSyntaxSection.register("Define Condition", CustomConditionSection.class, "[(1¦local)] condition <.+>",
				"[(1¦local)] condition", "[(1¦local)] %*classinfos% property condition <.+>");
	}
	...
	static {
		...
		Optional<SyntaxElementInfo<? extends Condition>> info = Skript.getConditions().stream()
				.filter(i -> i.c == CustomCondition.class).findFirst();
	...
	protected boolean init(Literal<?>[] args, int matchedPattern, SkriptParser.ParseResult parseResult,
		...
		case 2:
			what = parseResult.regexes.get(0).group();
			String type = Arrays.stream(((Literal<ClassInfo>) args[0]).getArray()).map(ClassInfo::getCodeName)
					.map(codeName -> {
			...
			register(ConditionSyntaxInfo.create(script, "%" + type + "% (isn't|is not|aren't|are not) " + what, 1, true,
					true));
			break;
		}
		...
		return handleEntriesAndSections(node, entryNode -> false, sectionNode -> {
