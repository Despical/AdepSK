...
public abstract class CustomSyntaxSection<T extends CustomSyntaxSection.SyntaxData> extends SelfRegisteringSkriptEvent {
	...
	public static class DataTracker<T> {
		...
		public void recomputePatterns() {
			patterns = primaryData.entrySet().stream().map(Map.Entry::getValue).map(Map::keySet).flatMap(Set::stream)
	...
	protected static boolean handleEntriesAndSections(SectionNode node, Predicate<EntryNode> entryHandler,
		...
		for (Node subNode : node) {
			...
			if (subNode instanceof EntryNode) {
				if (!entryHandler.test(((EntryNode) subNode))) {
					Skript.error(
							String.format("Unexpected entry '%s'. Check whether it's spelled correctly or remove it.",
									subNode.getKey()));
					ok = false;
				}
				...
				if (!sectionHandler.test(((SectionNode) subNode))) {
					Skript.error(
							String.format("Unexpected section '%s'. Check whether it's spelled correctly or remove it.",
									subNode.getKey()));
					ok = false;
				}
			} else {
	...
	protected final void register(T data) {
		...
		getDataTracker().getPrimaryData().computeIfAbsent(data.getScript(), f -> new HashMap<>()).put(pattern, data);
