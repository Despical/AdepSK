...
import me.adeptr.adepsk.skriptmirror.skript.reflect.ExprJavaCall;
...
public class CondExpressionStatement extends Condition {
	...
	private boolean isTruthy(Object o) {
		return o != Boolean.FALSE && o != null && (!(o instanceof Number)
				|| !(((Number) o).doubleValue() == 0 || Double.isNaN(((Number) o).doubleValue())));
	}
	...
	protected TriggerItem walk(Event e) {
		if (isAsynchronous) {
			CompletableFuture.runAsync(() -> check(e), threadPool)
					.thenAccept(res -> Bukkit.getScheduler().runTask(Main.getInstance(), () -> {
						if (getNext() != null) {
