package me.adeptr.adepsk.skriptmirror.skript;
...
import me.adeptr.adepsk.skriptmirror.Null;
import me.adeptr.adepsk.skriptmirror.ObjectWrapper;
import me.adeptr.adepsk.skriptmirror.skript.custom.CustomImport;
...
public class Types {
	static {
		Classes.registerClass(new ClassInfo<>(Event.class, "event").user("events?").parser(new Parser<Event>() {
			@Override
		...
		Classes.registerClass(
				new ClassInfo<>(JavaType.class, "javatype").user("javatypes?").parser(new Parser<JavaType>() {
					@Override
					...
					@Override
					protected JavaType deserialize(Fields fields)
							throws StreamCorruptedException, NotSerializableException {
						try {
							return new JavaType(
									LibraryLoader.getClassLoader().loadClass((String) fields.getObject("type")));
		...
		Classes.registerClass(new ClassInfo<>(Null.class, "null").parser(new Parser<Null>() {
			@Override
		...
		Classes.registerClass(new ClassInfo<>(ObjectWrapper.class, "javaobject").user("javaobjects?")
				.parser(new Parser<ObjectWrapper>() {
					...
					public String toString(ObjectWrapper o, int flags) {
						if (o instanceof ObjectWrapper.OfArray) {
							return Arrays.stream(((ObjectWrapper.OfArray) o).get()).map(Classes::toString)
