package me.adeptr.adepsk.skriptmirror.skript.reflect;
...
import me.adeptr.adepsk.skriptmirror.*;
import me.adeptr.adepsk.skriptmirror.util.JavaUtil;
import me.adeptr.adepsk.skriptmirror.util.SkriptMirrorUtil;
import me.adeptr.adepsk.skriptmirror.util.SkriptUtil;
import me.adeptr.adepsk.skriptmirror.util.StringSimilarity;
...
public class ExprJavaCall<T> implements Expression<T> {
	...
	/**
	 * A regular expression that captures potential descriptors without actually
	 * validating the descriptor. This is done both for performance reasons and to
	 * provide more helpful error messages when using a malformed descriptor.
	 */
	private static final String LITE_DESCRIPTOR = "[^.]+\\b";

	static {
		// noinspection unchecked
		Skript.registerExpression(ExprJavaCall.class, Object.class, ExpressionType.PATTERN_MATCHES_EVERYTHING,
	...
	public T getSingle(Event e) {
		...
		if (rawArgs != null) {
			if (rawArgs instanceof ExpressionList && rawArgs.getAnd()) {
				// In a 'comma/and' separated list, manually unwrap each expression and convert
				// nulls to Null wrappers
				// This ensures that expressions that return null do not change the arity of the
				// invoked method
				arguments = Arrays.stream(((ExpressionList) rawArgs).getExpressions())
						.map(SkriptUtil.unwrapWithEvent(e)).map(SkriptMirrorUtil::reifyIfNull).toArray(Object[]::new);
			} else if (rawArgs.isSingle()) {
				// A special case of the above, since a single argument will not be wrapped in a
				// list
				// Directly wrap the argument in an array to ensure the unary method is invoked
				arguments = new Object[] { rawArgs.getSingle(e) };
			} else {
				// If the user is using a non-single non-list expression, assume the number of
	...
	public Class<?>[] acceptChange(Changer.ChangeMode mode) {
		if (type == CallType.FIELD && (mode == Changer.ChangeMode.SET || mode == Changer.ChangeMode.DELETE)) {
	...
	private Collection<MethodHandle> createCallSite(Descriptor e) {
		...
		switch (type) {
		case FIELD:
			return JavaUtil.fields(javaClass).filter(f -> f.getName().equals(e.getName()))
					.peek(f -> f.setAccessible(true))
					.flatMap(JavaUtil
							.propagateErrors(f -> Stream.of(LOOKUP.unreflectGetter(f), LOOKUP.unreflectSetter(f))))
					.filter(Objects::nonNull).limit(2).collect(Collectors.toList());
		case METHOD:
			...
			return methodStream.peek(m -> m.setAccessible(true)).map(JavaUtil.propagateErrors(LOOKUP::unreflect))
					.filter(Objects::nonNull).collect(Collectors.toList());
		case CONSTRUCTOR:
			return JavaUtil.constructors(javaClass).peek(c -> c.setAccessible(true))
					.map(JavaUtil.propagateErrors(LOOKUP::unreflectConstructor)).filter(Objects::nonNull)
	...
	private T invoke(Object target, Object[] arguments, Descriptor baseDescriptor) {
		...
		if (!descriptor.getJavaClass().isAssignableFrom(targetClass)) {
			error(String.format("Incompatible %s call: %s on %s", type, descriptor,
					SkriptMirrorUtil.getDebugName(targetClass)));
			return null;
		}

		// Copy arguments so that the original array isn't modified by type conversions
		// For instance methods, the target of the call must be added to the start of
		// the arguments array
		...
		if (!method.isPresent()) {
			error(String.format("No matching %s: %s%s", type, descriptor, argumentsMessage(arguments)));
		...
		try {
			...
			error(throwable, String.format("%s %s%s threw a %s: %s%n", type, descriptor, argumentsMessage(arguments),
		...
		if (converted == null) {
			String toClasses = Arrays.stream(types).map(SkriptMirrorUtil::getDebugName)
					.collect(Collectors.joining(", "));
			error(String.format("%s %s%s returned %s, which could not be converted to %s", type, descriptor,
					argumentsMessage(arguments), argumentsToString(returnedValue), toClasses));
			return null;
		}
		...
		if (hasDynamicDescriptor()) {
			...
			try {
				...
				if (parsedDescriptor == null) {
					error(String.format("Invalid dynamic descriptor %s (%s)", dynamicDescriptor.toString(e, false),
							desc));
					return null;
				}
	...
	private Optional<MethodHandle> findCompatibleMethod(Descriptor descriptor, Object[] args) {
		return getCallSite(descriptor).stream().filter(mh -> matchesArgs(args, mh)).findFirst();
	}
	...
	private static boolean matchesArgs(Object[] args, MethodHandle mh) {
		...
		// If the method has varargs, make sure args has the minimum arity (exclude the
		// varargs parameter)
	...
	private static boolean canCoerceType(Object o, Class<?> to) {
		...
		if (o instanceof String && (to == char.class || to == Character.class) && ((String) o).length() == 1) {
			return true;
		}
	...
	private static Object[] convertTypes(MethodHandle mh, Object[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			// varargs parameters are always arrays, but the method handle expects the array
			// to be spread before called
			if (loopAtVarargs) {
				param = params[varargsIndex].getComponentType();
			} else {
				param = params[i];
			...
			// coerce arrays of numeric types
			if (param.isArray() && JavaUtil.getArrayDepth(param) == JavaUtil.getArrayDepth(args[i].getClass())
			...
			// coerce single character strings to chars
			if (args[i] instanceof String && (param == char.class || param == Character.class)) {
	...
	private void suggestParameters(Descriptor descriptor) {
		...
		List<String> matches = members.filter(e -> e.getName().equals(guess)).map(Executable::getParameters)
				.map(params -> Arrays.stream(params).map(Parameter::getType).map(Class::getTypeName)
						.collect(Collectors.joining(",")))
				.collect(Collectors.toList());

		if (!matches.isEmpty()) {
			directError(String.format("Did you pass the wrong parameters? Here are the parameter signatures for %s:",
					guess));
	...
	private void suggestTypo(Descriptor descriptor) {
		...
		List<String> matches = members.map(Member::getName).filter(m -> !m.equals(guess)) // this would be a parameter
																							// mismatch, not a typo
				.distinct().map(m -> StringSimilarity.compare(guess, m, 3)).filter(Objects::nonNull).sorted()
				.map(StringSimilarity.Result::getRight).collect(Collectors.toList());

		if (!matches.isEmpty()) {
			directError(
					String.format("Did you misspell the %s? You may have meant to type one of the following:", type));
	...
	private static String argumentsToString(Object... arguments) {
		return Arrays.stream(arguments)
				.map(arg -> String.format("%s (%s)", Classes.toString(arg),
						SkriptMirrorUtil.getDebugName(SkriptMirrorUtil.getClass(arg))))
				.collect(Collectors.joining(", "));
	}
}
