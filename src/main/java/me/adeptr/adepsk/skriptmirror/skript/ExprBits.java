...
public class ExprBits extends SimpleExpression<Number> {
	...
	protected Number[] get(Event e) {
		...
		return Arrays.stream(numbers.getArray(e)).map(l -> (l.longValue() & mask) >>> f.intValue())
				.toArray(Number[]::new);
	}
	...
	public String toString(Event e, boolean debug) {
		return String.format("the bits %s to %s of %s", from.toString(e, debug), to.toString(e, debug),
				numbers.toString(e, debug));
	}
	...
	public boolean init(Expression<?>[] exprs, int matchedPattern, Kleenean isDelayed,
		...
		// it just so happens that we can use matchedPattern to determine the offset of
		// the arguments
