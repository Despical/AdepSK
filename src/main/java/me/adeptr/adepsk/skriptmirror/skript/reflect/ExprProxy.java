...
import me.adeptr.adepsk.skriptmirror.FunctionWrapper;
import me.adeptr.adepsk.skriptmirror.JavaType;
import me.adeptr.adepsk.skriptmirror.LibraryLoader;
import me.adeptr.adepsk.skriptmirror.skript.Consent;
...
public class ExprProxy extends SimpleExpression<Object> {
	...
	protected Object[] get(Event e) {
		...
		handler.variablesIterator(e).forEachRemaining(pair -> {
		...
		return new Object[] {
				Proxy.newProxyInstance(
						LibraryLoader.getClassLoader(), Arrays.stream(interfaces.getArray(e))
								.map(JavaType::getJavaClass).filter(Class::isInterface).toArray(Class[]::new),
	...
	private static class VariableInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] methodArgs) throws Throwable {
			...
			Arrays.stream(functionArgs).map(arg -> new Object[] { arg }).forEach(params::add);
			params.add(new Object[] { proxy });
			Arrays.stream(methodArgs).map(arg -> new Object[] { arg }).forEach(params::add);

			FunctionEvent functionEvent = new FunctionEvent(null);

			return function.execute(functionEvent,
					params.stream().limit(SkriptReflection.getParameters(function).length).toArray(Object[][]::new));
		}
	}
	...
	public String toString(Event e, boolean debug) {
		return String.format("proxy of %s from %s", interfaces.toString(e, debug), handler.toString(e, debug));
	}
