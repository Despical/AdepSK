...
import me.adeptr.adepsk.skriptmirror.WrappedEvent;
...
public class EvtByReflection extends SkriptEvent {
	...
	static {
		listeners = Arrays.stream(EventPriority.values()).mapToInt(EventPriority::ordinal)
				.mapToObj(PriorityListener::new).toArray(PriorityListener[]::new);
	}
	...
	private static void registerEvent(Class<? extends Event> event, EventPriority priority, boolean ignoreCancelled) {
		...
		if (!events.contains(event)) {
			events.add(event);
			Bukkit.getPluginManager().registerEvent(event, listener, priority, executor, Main.getInstance(),
					ignoreCancelled);
		}
	}
	...
	public boolean init(Literal<?>[] args, int matchedPattern, SkriptParser.ParseResult parseResult) {
		classes = Arrays.stream(((Literal<JavaType>) args[0]).getArray()).map(JavaType::getJavaClass)
				.toArray(Class[]::new);
