...
public class CustomCondition extends Condition {
	...
	public boolean check(Event e) {
		...
		if (checker == null) {
			Skript.error(String.format("The custom condtion '%s' no longer has a check handler.", which.getPattern()));
			return false;
		}
	...
	private boolean checkByProperty(Event e, Trigger checker) {
		return exprs[0].check(e, o -> {
			...
			ConditionCheckEvent conditionEvent = new ConditionCheckEvent(e, localExprs, which.getMatchedPattern(),
					parseResult);
			SkriptReflection.copyVariablesMap(parseEvent, conditionEvent);
			checker.execute(conditionEvent);
