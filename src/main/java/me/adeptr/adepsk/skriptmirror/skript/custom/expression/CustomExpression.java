...
import me.adeptr.adepsk.skriptmirror.skript.custom.SyntaxParseEvent;
import me.adeptr.adepsk.skriptmirror.util.JavaUtil;
import me.adeptr.adepsk.skriptmirror.util.SkriptReflection;
...
public class CustomExpression<T> implements Expression<T> {
	...
	public T[] getAll(Event e) {
		...
		if (getter == null) {
			Skript.error(String.format("The custom expression '%s' no longer has a get handler.", which.getPattern()));
	...
	private T[] getByStandard(Event e, Trigger getter) {
		...
		if (expressionEvent.getOutput() == null) {
			Skript.error(String.format("The get handler for '%s' did not return.", which.getPattern()));
			return JavaUtil.newArray(superType, 0);
		}
	...
	private T[] getByProperty(Event e, Trigger getter) {
		...
		for (Object o : exprs[0].getArray(e)) {
			...
			ExpressionGetEvent expressionEvent = new ExpressionGetEvent(e, localExprs, which.getMatchedPattern(),
					parseResult);
			SkriptReflection.copyVariablesMap(parseEvent, expressionEvent);
			getter.execute(expressionEvent);
			...
			if (exprOutput == null) {
				Skript.error(String.format("The get handler for '%s' did not return for the value %s",
	...
	public boolean isSingle() {
		return !which.isAlwaysPlural() && Arrays.stream(which.getInheritedSingles()).mapToObj(i -> exprs[i])
				.filter(Objects::nonNull).allMatch(Expression::isSingle);
	}
	...
	public Class<?>[] acceptChange(Changer.ChangeMode mode) {
		if (CustomExpressionSection.changerHandlers.containsKey(which)
			...
			return CustomExpressionSection.changerTypes.getOrDefault(which, Collections.emptyMap()).getOrDefault(mode,
	...
	public void change(Event e, Object[] delta, Changer.ChangeMode mode) {
		...
		if (changer == null) {
			Skript.error(String.format("The custom expression '%s' no longer has a %s handler.", which.getPattern(),
					mode.name()));
		} else {
			ExpressionChangeEvent changeEvent = new ExpressionChangeEvent(e, exprs, which.getMatchedPattern(),
					parseResult, delta);
	...
	public boolean init(Expression<?>[] exprs, int matchedPattern, Kleenean isDelayed,
		...
		this.exprs = Arrays.stream(exprs).map(SkriptUtil::defendExpression).toArray(Expression[]::new);
		...
		if (parseHandler != null) {
			SyntaxParseEvent event = new SyntaxParseEvent(this.exprs, matchedPattern, parseResult,
