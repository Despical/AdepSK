package me.adeptr.adepsk.skriptmirror.skript.custom;
...
public class CustomImport {
	private static Pattern IMPORT_STATEMENT = Pattern
			.compile("(" + SkriptMirrorUtil.PACKAGE + ")(?:\\s+as (" + SkriptMirrorUtil.IDENTIFIER + "))?");
	...
	static {
		...
		Optional<ExpressionInfo<?, ?>> info = StreamSupport
				...
				.filter(i -> i.c == ImportHandler.class).findFirst();
	...
	// Most scripts are associated with files, but according to Skript, file-less
	// configs may also be loaded.
	...
	private static void registerImport(String rawStatement, File script) {
		...
		imports.computeIfAbsent(script, s -> new HashMap<>()).compute(importName, (name, oldClass) -> {
			if (oldClass != null) {
				Skript.warning(String.format("%s is already mapped to %s. It will not be remapped to %s.", name,
						oldClass.getJavaClass(), javaClass));
				return oldClass;
			}
	...
	private static void updateImports() {
		String[] patterns = imports.values().stream().flatMap(m -> m.keySet().stream()).distinct()
				.toArray(String[]::new);
