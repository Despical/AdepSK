...
public class EffMakeGUI extends EffectSection {
	...
	static {
		Registry.newEffect(EffMakeGUI.class, "(make|format) next gui [slot] (with|to) %itemstack%",
				"(make|format) gui [slot] %strings/numbers% (with|to) %itemstack%",
				"(un(make|format)|remove) next gui [slot]", "(un(make|format)|remove) gui [slot] %strings/numbers%",
	...
	public void execute(Event e) {
		...
		if (gui == null)
			return;
		if (type > 1) {
			Object[] slots = this.slot != null ? this.slot.getArray(e)
					: type == 4 ? null : new Object[] { gui.nextInvertedSlot() };
