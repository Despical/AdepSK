...
import me.adeptr.adepsk.gui.GUI;
import me.adeptr.adepsk.gui.SkriptGUIEvent;
import me.adeptr.adepsk.util.CommandUtils;
import me.adeptr.adepsk.util.EffectSection;
import me.adeptr.adepsk.util.EvalFunction;
...
public class EffFormatGUI extends EffectSection {
	static {
		...
		Registry.newEffect(EffFormatGUI.class,
			...
			"(format|create|make) [a] gui slot [%-numbers%] of %players% with %itemstack% to [close then] (run|exe[cute]) %commandsender% command %string% [[(,| and)] (using|with) %-" + cr +
			"% [(button|click|action)]][[(,| and)] (using|with) cursor [item] %-itemstack%]", "(format|create|make) [a] gui slot [%-numbers%] of %players% with %itemstack% to [close then] (run|exe[cute]) function <(.+)>\\([<.*?>]\\)[[(,| and)] (using|with) %-" + cr +
			"% [(button|click|action)]][[(,| and)] (using|with) cursor [item] %-itemstack%]", "(format|create|make) [a] gui slot [%-numbers%] of %players% with %itemstack% to (run|exe[cute]) [gui [click]] event");
	}
	...
	public boolean init(Expression<?>[] arg, int arg1, Kleenean arg2, ParseResult arg3) {
		...
		if (hasSection()) {
			...
			Skript.error(
					"You can't execute a blank code in this effect. In case you want to format a unstealable item, use 'format gui slot ... to do nothing' instead.");
			return false;
		}
		int max = arg.length;
	...
	public String toString(@Nullable Event e, boolean arg1) {
		return "format a gui slot " + (s != null ? s.toString(e, arg1) : -1) + " of " + p.toString(e, arg1) + " with "
				+ i.toString(e, arg1);
	}
		...
		if (this.p.getArray(e) != null && this.i.getSingle(e) != null) {
			...
			for (int x = 0; x < slots.length; x++)
				for (int y = 0; y < p.length; y++) {
					...
					if (slots[x].intValue() >= -2 && slots[x].intValue() < inv.getSize()
						...
						GUI gui = new GUI(rn, (i2 != null && i2.getSingle(e) != null ? i2.getSingle(e) : null),
								(ct != null ? getFromObject(ct.getSingle(e)) : null));
						if (runEvent)
