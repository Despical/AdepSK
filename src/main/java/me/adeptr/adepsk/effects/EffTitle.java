...
public class EffTitle extends Effect {
	...
	static {
		Registry.newEffect(EffTitle.class,
				"send title %string% with subtitle %string% to %players% with %integer% fadein and %integer% fadeout for %integer%");
	}
	...
	protected void execute(Event arg0) {
		...
		try {
			for (Player p : player.getAll(arg0)) {
				Main.getInstance().getTitle().sendTitle(p, title.getSingle(arg0), subtitle.getSingle(arg0),
						fadein.getSingle(arg0), fadeout.getSingle(arg0), time.getSingle(arg0));
			}
