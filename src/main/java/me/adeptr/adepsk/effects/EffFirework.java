...
public class EffFirework extends Effect {
	...
	static {
		Registry.newEffect(EffFirework.class,
				"(launch|deploy) [%-strings%] firework[s] at %locations% [with] (duration|timed|time) %number% [colo[u]r[ed] (%-strings%|%-color%)]");
	}
			...
			if (multiColours != null || finalColour == null) {
				for (String s : multiColours.getAll(e)) {
					if (type == null) {
						...
						meta.addEffect(FireworkEffect.builder()
								.withColor(ch.njol.skript.util.Color.byName(s.toString()).getBukkitColor()).with(t)
								.build());
					}
				}
			} else {
