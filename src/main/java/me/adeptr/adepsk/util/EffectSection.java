...
public abstract class EffectSection extends Condition {
	...
	public EffectSection() {
		...
		hasIfOrElseIf = StringUtils.startsWithIgnoreCase(n.getKey(), "if ")
				|| StringUtils.startsWithIgnoreCase(n.getKey(), "else if ");
		// The comment value of a note is protected, so it is needed but not really
		// necessary tho.
		...
		ReflectionUtils.setField(SectionNode.class, section, "nodes",
				ReflectionUtils.getField(SectionNode.class, n, "nodes"));
		// Then it will clear the nodes from the current node, so Skript won't parse it
		// (you need to parse then later).
	...
	/**
	 * It will load the section of this if any. It must be used before
	 * {@link #runSection(Event)}.
	 * 
	 * @param setNext - Set the next trigger of this loading section, to actual next
	 *                of this effect.
	 */
	...
	/**
	 * It will load the section of this if any and then it will parse as in specific
	 * event. Basically it will call
	 * {@link ScriptLoader#setCurrentEvent(String, Class[])}, parse the current
	 * section, and then set the current event back to the previous one. Useful to
	 * load a code from event X and parse as Y, allowing to use syntaxes that work
	 * on it.
	 *
	...
	/**
	 * Check if this has any section (basically check if it is inline condition or
	 * Condtional)
	 * 
	...
	/**
	 * Run the section. <b>Note</b>: You must {@link #loadSection(boolean)} first
	 * and you should run it with same event from {@link #execute(Event)}
	 * 
	 * @param e - The event
	 */
	...
	/**
	 * It will check in case the effect wasn't used with 'if/else if' before <code>
	 *     do something:
	 *     	send "Everything fine"
	 *     if do something:
	 *     	send "Not ok, it will send a default message and return false"
	 * </code> It needs to be used in
	 * {@link #init(Expression[], int, Kleenean, SkriptParser.ParseResult)} method,
	 * like: <code>
	 *     public boolean init(...) {
	...
	/**
	 * The section node of {@link EffectSection}. It can return null if it was used
	 * after {@link #loadSection(boolean)} or if this doesn't have any section.
	 * 
	...
	/**
	 * A hacky method to fix wrong syntax inside of sections not beeing included in
	 * errors. Why? Because before parsing the effect itself, Skript starts a
	 * ParseLogHandler, then, in case the syntax returns true in
	 * {@link #init(Expression[], int, Kleenean, SkriptParser.ParseResult)}, The
	 * LogHander will ignore all errors that was sent in this method. So to fix
	 * that, it stops the lasts ParseLogHandlers to not conflict with.
	 * 
	...
	private void stopLog(RetainingLogHandler logger) {
		...
		while (it.hasNext()) {
			...
			if (l instanceof ParseLogHandler)
				toStop.add(l);
			else // We can only stop the lasts handler, this prevent in case the last is not what
					// we want.
				break;
		}
