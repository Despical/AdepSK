...
public class Registry {
	...
	public static void newProperty(Class<?> expr, int amount, String property, String from) {
		newExpression(expr, amount, ExpressionType.PROPERTY, "[the] " + property + " of %" + from + "%",
				"%" + from + "%'[s] " + property);
	}
	...
	private static <E extends Expression<T>, T> void newExpression(Class<?> expression, int amount, ExpressionType type,
			String... syntaxes) {
