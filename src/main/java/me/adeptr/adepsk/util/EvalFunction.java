...
/**
 * A util class to evaluate functions. It's useful when you want to get a
 * function ready, and run the function in another event but using values of the
 * caller event. It requires {@link ReflectionUtils}
 * 
 * @author Tuke_Nuke
 */
...
public class EvalFunction {
	...
	// Used to make compatible with bensku's new parser (even tho the latest version
	...
	static {
		...
		if (parserInstanceClass != null) {
			newParser = ReflectionUtils.getConstructor(SkriptParser.class, parserInstanceClass, String.class, int.class,
					ParseContext.class);
			// newParserInstance = ReflectionUtils.getField(parserInstanceClass, null,
			// "DUMMY");
		}
	}
	...
	/**
	 * It get the values of a function from the event. You need to {@link #run()} to
	 * call the functions after get these values.
	 * 
	 * @param e - The Event
	...
	public EvalFunction getParemetersValues(Event e) {
		...
		if (func != null) {
			...
			int max = f.getParameters().length < parameters.length ? f.getParameters().length
					: parameters.length < 1 ? 1 : parameters.length;
			if (values != null) {// In case it run in a loop, it will return a new function to each one have it
									// own parameters.
	...
	/**
	 * Run the function. You need to {@link #getParemetersValues(Event)} before
	 * running this.
	 * 
	...
	/**
	 * It gets a ParserInstance from a effect/condition/expression. It is necessary
	 * in case it is running the new parser and it can't parse some objects with
	 * dummy ParserInstance (e.g loop-value).
	 * 
	...
	private SkriptParser getSkriptParser(String expr) {
		if (newParser != null) {
			if (newParserInstance == null)
				newParserInstance = ReflectionUtils
						.newInstance(ReflectionUtils.getClass("ch.njol.skript.lang.parser.ParserInstance"));
			return ReflectionUtils.newInstance(newParser, newParserInstance, expr, SkriptParser.ALL_FLAGS,
