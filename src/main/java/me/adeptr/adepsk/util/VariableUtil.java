...
public class VariableUtil {
	...
	/**
	 * Some hacking methods to copy variables from one event, and paste to another.
	 * It allows to run the section using the same variables when was making the gui
	 * 
	...
	public Object copyVariables(Event from) {
		if (from != null && map.containsKey(from)) {
			...
			if (newVariablesMap == null)
				return null;
			Map<String, Object> single = ReflectionUtils.getField(newVariablesMap.getClass(), newVariablesMap,
					"hashMap");
	 ...
	 * @param variables - The object VariableMap returned in
	 *                  {@link #copyVariables(Event)}
	 */
