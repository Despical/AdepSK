...
public class CuboidIterator extends CheckedIterator<Block> {
	...
	public CuboidIterator(final Chunk chunk) {
		super(new CuboidRegion(chunk.getBlock(0, 0, 0).getLocation(),
				chunk.getBlock(15, chunk.getWorld().getMaxHeight() - 1, 15).getLocation()).iterator(),
				new NullableChecker<Block>() {
					@Override
					public boolean check(Block block) {
						return new CuboidRegion(chunk.getBlock(0, 0, 0).getLocation(),
								chunk.getBlock(15, chunk.getWorld().getMaxHeight() - 1, 15).getLocation())
										.checkHas(block.getLocation().toVector());
					}
				});
	}
}
