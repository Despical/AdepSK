...
public class InventoryUtils {
	...
	public static Inventory getOpositiveInventory(InventoryView view, Inventory inv) {
		if (view == null || inv == null)
			return null;
		return view.getTopInventory().equals(inv) ? view.getBottomInventory()
				: view.getBottomInventory().equals(inv) ? view.getTopInventory() : null;
	}
	...
	public static int getSlotTo(Inventory invTo, ItemStack i) {
		if (i != null && invTo.first(i.getType()) >= 0)
			for (int x = invTo.first(i.getType()); x < invTo.getSize(); x++)
				if (invTo.getItem(x) != null && invTo.getItem(x).getData().equals(i.getData())
						&& invTo.getItem(x).getAmount() < invTo.getItem(x).getMaxStackSize())
					return x;
	...
	public static int getInvertedSlotTo(Inventory invTo, ItemStack i) {
		for (int x = 8; x >= 0; x--)
			if ((invTo.getItem(x) == null)
					|| (invTo.getItem(x) != null && invTo.getItem(x).getData().equals(i.getDurability())
							&& invTo.getItem(x).getAmount() < invTo.getItem(x).getMaxStackSize()))
				return x;
