...
@Examples({ "if yaml node \"listnode\" from \"example\" has list:", "\tloop yaml list \"listnode\" from \"example\":",
		"\t\tbroadcast \"%loop-val%\"", })
@Since("1.3")
public class CondNodeHasList extends Condition {
		...
		return path.check(event, new Checker<String>() {
			...
			public boolean check(final String s) {
				if (!Main.YAML_STORE.containsKey(file.getSingle(event)))
					return false;
				Object o = Main.YAML_STORE.get(file.getSingle(event)).getProperty(path.getSingle(event));
	...
	public String toString(final @Nullable Event event, final boolean debug) {
		return "yaml path " + path.toString(event, debug) + " in " + file.toString(event, debug)
				+ (isNegated() ? " is not a list " + " " : "is a list");
	}
