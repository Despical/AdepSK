package me.adeptr.adepsk.yaml.skript;
...
@Examples({ "set the comments of yaml node \"test\" from \"config\" to \"First line\" and \"Second line\"",
		"delete the comments of yaml node \"test\" from \"config\"", " ",
		"set {_header::*} to \"First line\" and \"Second line\"",
		"set the comments at the top of \"config\" to {_header::*}", "delete the comments at the top of \"config\"",
...
public class ExprYamlComments extends SimpleExpression<Object> {
	...
	public String toString(@Nullable Event event, boolean b) {
		return state.toString().toLowerCase()
				+ (state == States.COMMENT ? " for path " + this.paths.toString(event, b) : "") + " from yaml "
				+ this.file.toString(event, b);
	}
	...
	protected Object[] get(Event event) {
		...
		if (!Main.YAML_STORE.containsKey(name)) {
			Main.warn("No yaml by the name '" + name + "' has been loaded");
			return null;
		}

		YAMLProcessor config = Main.YAML_STORE.get(name);
				...
				for (String p : paths) {
					if (!p.contains(".")) {
						if (config.getMap().containsKey(p))
							...
							Main.warn("'" + p + "' is not a valid path in '" + name + "'");
					} else {
						Main.warn("Comments can only be added to root paths not '" + p + "' in '" + name + "'");
					}
				}
