...
@Examples({ "set {_list::*} to the currently loaded yaml files", "broadcast \"%{_list::*}%\"", " ",
		"loop the loaded yaml", "\tbroadcast loop-value", " ",
		"loop the loaded yaml from directory \"plugins\\skript-yaml\"", "\tbroadcast loop-value", " ",
		"loop the loaded yaml directories", "\tbroadcast loop-value" })
@Since("1.0.0")
public class ExprAllLoadedYaml extends SimpleExpression<String> {
	...
	public String toString(@Nullable Event event, boolean debug) {
		if (matchedPattern == 0)
			return "(the|all [of the]) [currently] loaded y[a]ml [files]"
					+ (directory != null ? " from directory " + directory.toString(event, debug) : "");
		else
	...
	protected String[] get(Event event) {
		if (matchedPattern == 0) {
			if (directory == null) {
				if (Main.YAML_STORE.isEmpty())
					return null;
				return Main.YAML_STORE.keySet().toArray(new String[Main.YAML_STORE.keySet().size()]);
			} else {
	...
	private String[] getYamlFromDirectories(String... directories) {
		...
		for (Iterator<Entry<String, YAMLProcessor>> it = Main.YAML_STORE.entrySet().iterator(); it.hasNext();) {
