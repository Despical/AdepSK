...
@Examples({ "set skript-yaml value \"test.test\" from \"config\" to \"test\"", " ",
		"yaml path \"test.test\" in \"config\" has value:", "\tbroadcast \"value exists\"", })
@Since("1.1.0")
public class CondNodeHasValue extends Condition {
		...
		return path.check(event, new Checker<String>() {
			...
			public boolean check(final String s) {
				if (!Main.YAML_STORE.containsKey(file.getSingle(event)))
					return false;
				if (path.isSingle())
					return (Main.YAML_STORE.get(file.getSingle(event)).getProperty(path.getSingle(event)) != null);
				else {
					String[] paths = (String[]) path.getAll(event);
					boolean check;
					for (String p : paths) {
						check = (Main.YAML_STORE.get(file.getSingle(event)).getProperty(p) != null);
						if (!check) {
							return false;
						}
					}
	...
	public String toString(final @Nullable Event event, final boolean debug) {
		return "yaml path " + path.toString(event, debug) + " in " + file.toString(event, debug)
				+ (isNegated() ? (path.isSingle() ? " does not have a value" : " do not have values") + " "
						: "has a value");
	}
