...
import me.adeptr.adepsk.yaml.AsyncEffectOld;
import me.adeptr.adepsk.yaml.utils.SkriptYamlUtils;
...
public class EffDeleteYamlFromDirectory extends AsyncEffectOld {
	...
	protected void execute(@Nullable Event event) {
		for (String name : this.directories.getAll(event)) {
			for (File yamlFile : SkriptYamlUtils.directoryFilter(StringUtil.checkSeparator(name), mark == 1,
					"Delete")) {
				if (matchedPattern == 0) {
					yamlFile.delete();
				} else {
					...
					if (Main.YAML_STORE.containsKey(n)) {
						Main.YAML_STORE.get(n).getFile().delete();
						Main.YAML_STORE.remove(n);
					}
				}
			}
		}
	...
	public String toString(@Nullable Event event, boolean b) {
		return "delete all" + (matchedPattern == 0 ? " loaded " : " ") + "yaml from"
				+ (mark == 1 ? " non-relative " : " ") + "director" + (directories.isSingle() ? "y " : "ies ")
				+ this.directories.toString(event, b) + (matchedPattern == 2 ? " using the filename as the id" : "");
	}
