...
import me.adeptr.adepsk.yaml.utils.yaml.YAMLFormat;
...
@Examples({ "#Both examples produce the same id for use in other effects/expressions",
...
public class EffLoadYaml extends Effect {
	...
	static {
		Skript.registerEffect(EffLoadYaml.class, "[re]load [(1¦non[(-| )]relative)] [y[a]ml] %strings%",
	...
	protected void execute(@Nullable Event event) {
		...
		if (matchedPattern == 1) {
			if (!this.file.isSingle()) {
				Main.warn("[Load Yaml] Input has to be single if using a custom id");
				return;
			}
	...
	private void load(String name, Event event) {
		...
		try {
			...
			Main.error("[Load Yaml] " + error.getMessage() + " (" + name + ")");
			return;
		}
		...
		try {
			...
			if (matchedPattern == 0)
				...
				n = name;
			Main.YAML_STORE.put(n, yaml);
		}
	}

	@Override
	public String toString(@Nullable Event event, boolean b) {
		return "[re]load yaml" + (mark == 1 ? " non-relative " : " ") + this.file.toString(event, b)
				+ (id != null ? " as " + this.id.toString(event, b)
						: (matchedPattern == 2 ? " using the file path as the id" : ""));
	}
