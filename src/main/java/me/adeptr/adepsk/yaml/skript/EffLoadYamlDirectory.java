...
@Examples({ "#This isn't something you would really want to do, or is it?",
		"load all yaml from directory \"/plugins/skript-yaml/test\"", "loop all of the currently loaded yaml files:",
		"\tloop yaml nodes \"\" from loop-value-1:", "\t\tloop yaml nodes loop-value-2 from loop-value-1:",
		...
		for (String name : this.directories.getAll(event)) {
			for (File yamlFile : SkriptYamlUtils.directoryFilter(StringUtil.checkSeparator(name), mark == 1, "Load")) {
				...
				try {
					...
					if (matchedPattern == 1)
						Main.YAML_STORE.put(StringUtil.stripExtention(yamlFile.getName()), yaml);
					else
						Main.YAML_STORE.put(StringUtil.checkLastSeparator(name) + yamlFile.getName(), yaml);
				}
			}
		}
	}
