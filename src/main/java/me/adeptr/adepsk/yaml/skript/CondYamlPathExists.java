...
@Examples({ "set skript-yaml value \"test.test\" from \"config\" to \"test\"",
		"set skript-yaml value \"test2.test2\" from \"config\" to \"test\"", " ",
		"yaml path \"test.test\" and \"test2.test2\" in \"config\" exists:", "\tbroadcast \"this works\"",
		"yaml path \"test.test\" and \"boop.boop\" in \"config\" exists:", "\tbroadcast \"this will fail\"" })
@Since("1.2.1")
public class CondYamlPathExists extends Condition {
	...
	public boolean check(final Event event) {
		return path.check(event, new Checker<String>() {
			...
			public boolean check(final String s) {
				if (!Main.YAML_STORE.containsKey(file.getSingle(event)))
					return false;
				if (path.isSingle())
					return Main.YAML_STORE.get(file.getSingle(event)).getAllKeys().contains(path.getSingle(event));
				else {
					boolean check;
					for (String p : path.getAll(event)) {
						check = Main.YAML_STORE.get(file.getSingle(event)).getAllKeys().contains(p);
						if (!check) {
							return false;
						}
					}
	...
	public String toString(final @Nullable Event event, final boolean debug) {
		return "yaml path " + path.toString(event, debug) + " in " + file.toString(event, debug)
				+ (isNegated() ? (path.isSingle() ? " does" : " do") + " not exist" : "exist");
	}
