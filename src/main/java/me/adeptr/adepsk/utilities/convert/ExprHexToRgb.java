...
public class ExprHexToRgb extends SimpleExpression<String> {
	...
	protected String[] get(Event e) {
		...
		if (s.length() == 6) {
			...
			return new String[] { Integer.toString((c >> 16) & 0xFF) + ", " + Integer.toString((c >> 8) & 0xFF) + ", "
					+ Integer.toString((c) & 0xFF) };
		} else {
			Main.prSysE("Length must be 6. (FFFFFF)!", getClass().getSimpleName());
		}
		return null;
	}
