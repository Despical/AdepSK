...
public class ExprDateInner extends SimpleExpression<String> {
	...
	protected String[] get(Event e) {
		...
		try {
			LocalDateTime ldt = LocalDateTime.parse(d.toString(),
					DateTimeFormatter.ofPattern(new SimpleDateFormat().toPattern()));
			switch (ty) {
			case 0: {
			...
			case 9: {
				long ul = (id.getSingle(e).getTimestamp() / 1000L)
						- (ldt.toEpochSecond(ZoneOffset.ofTotalSeconds(ldt.getSecond())));
				while (ul > 59) {
					if (ul > 3600) {
						ul = (ul - 3600);
					} else {
