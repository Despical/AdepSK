...
import me.adeptr.adepsk.utilities.files.event.EvtFileCreation;
...
public class EffCreateFile extends Effect {
	...
	protected void execute(Event e) {
		...
		if (!efc.isCancelled()) {
			if (ty == 0) {
				try {
					Path fwn = Paths.get(pth.toString().replace(
							File.separator + pth.toString().substring(pth.toString().lastIndexOf(File.separator) + 1),
							""));
					if (!Files.exists(fwn)) {
					...
					Main.prSysE("File: '" + pth + "' already exists!", getClass().getSimpleName(), x);
				}
			} else {
				try {
					Files.createDirectories(pth);
				} catch (Exception x) {
					Main.prSysE("Directory: '" + pth + "' already exists!", getClass().getSimpleName(), x);
				}
			}
		}
	}
