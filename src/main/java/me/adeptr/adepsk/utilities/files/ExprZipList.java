...
public class ExprZipList extends SimpleExpression<String> {
	...
	protected String[] get(Event e) {
		...
		try (ZipFile zf = new ZipFile(pth)) {
			cl.addAll(zf.stream().collect(Collectors.toList()).stream()
					.map((Function<ZipEntry, String>) ZipEntry::toString).collect(Collectors.toList()));
			return cl.toArray(new String[cl.size()]);
		} catch (Exception x) {
			Main.prSysE("ZipFile: '" + pth + "' doesn't exist, or doesn't have write permission!",
