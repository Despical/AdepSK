...
public class EffZipFiles extends Effect {
	...
	protected void execute(Event e) {
		...
		for (String Spth : files.getAll(e)) {
			cl.add(new File(Main.getDefaultPath(Spth)));
		}
		...
		if (!efz.isCancelled()) {
			try {
				...
				Main.prSysE("ZipFile: '" + Fzip + "' doesn't exist!", getClass().getSimpleName(), x);
			} catch (Exception x) {
				Main.prSysE("Files: '" + Arrays.toString(s) + "' 1 or " + s.length + " Files don't exist!",
